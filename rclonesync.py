...
def bidirSync():
    ...
    listFileBase = LOCAL_WD + remotePathBase.replace(':', '_').replace(r'/',
                                                                       '_')  # '/home/<user>/.RCloneSyncWD/Remote__some_path_' or '/home/<user>/.RCloneSyncWD/Remote_'.
    ...
    if dryRun:
        ...
        if os.path.exists(
                localListFile):  # If dryrun, origianl LSL files are preserved and lsl's are done to the _DRYRUN files.
    ...
    if first_sync:
        ...
        if rcloneLSL(localPathBase, localListFile, excludes,
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL

        if rcloneLSL(remotePathBase, remoteListFile, excludes,
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL

        status, localNow = loadList(localListFile)
        if status: logging.error(
            printMsg("ERROR", "Failed loading local list file <{}>".format(localListFile))); return RTN_CRITICAL

        status, remoteNow = loadList(remoteListFile)
        if status: logging.error(
            printMsg("ERROR", "Failed loading remote list file <{}>".format(remoteListFile))); return RTN_CRITICAL
        ...
        for key in remoteNow:
            if key not in localNow:
                ...
                if rcloneCmd('copyto', src, dest, switches,
                             linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
    ...
    if checkAccess:
        ...
        if rcloneLSL(localPathBase, localChkListFile, ['--include', chkFile],
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_ABORT

        if rcloneLSL(remotePathBase, remoteChkListFile, ['--include', chkFile],
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_ABORT

        status, localCheck = loadList(localChkListFile)
        if status: logging.error(printMsg("ERROR", "Failed loading local check list file <{}>".format(
            localChkListFile))); return RTN_CRITICAL

        status, remoteCheck = loadList(remoteChkListFile)
        if status: logging.error(printMsg("ERROR", "Failed loading remote check list file <{}>".format(
            remoteChkListFile))); return RTN_CRITICAL
        ...
        if len(localCheck) < 1 or len(localCheck) != len(remoteCheck):
            ...
            for key in localCheck:
                ...
                if key not in remoteCheck:
                    logging.error(
                        printMsg("ERROR", "Failed access health test: Local key <{}> not found in remote".format(key),
                                 ""));
                    return RTN_CRITICAL
    ...
    if rcloneLSL(localPathBase, localListFileNew, excludes,
                 linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL

    remoteListFileNew = listFileBase + '_remoteLSL_new'
    if rcloneLSL(remotePathBase, remoteListFileNew, excludes,
                 linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
    ...
    if status:                    logging.error(
        printMsg("ERROR", "Failed loading prior local list file <{}>".format(localListFile))); return RTN_CRITICAL
    if len(localPrior) == 0:    logging.error(
        printMsg("ERROR", "Zero length in prior local list file <{}>".format(localListFile))); return RTN_CRITICAL

    status, remotePrior = loadList(remoteListFile)
    if status:                    logging.error(
        printMsg("ERROR", "Failed loading prior remote list file <{}>".format(remoteListFile))); return RTN_CRITICAL
    if len(remotePrior) == 0:    logging.error(
        printMsg("ERROR", "Zero length in prior remote list file <{}>".format(remoteListFile))); return RTN_CRITICAL

    status, localNow = loadList(localListFileNew)
    if status:                    logging.error(
        printMsg("ERROR", "Failed loading current local list file <{}>".format(localListFileNew))); return RTN_ABORT
    if len(localNow) == 0:        logging.error(
        printMsg("ERROR", "Zero length in current local list file <{}>".format(localListFileNew))); return RTN_ABORT

    status, remoteNow = loadList(remoteListFileNew)
    if status:                    logging.error(
        printMsg("ERROR", "Failed loading current remote list file <{}>".format(remoteListFileNew))); return RTN_ABORT
    if len(remoteNow) == 0:        logging.error(
        printMsg("ERROR", "Zero length in current remote list file <{}>".format(remoteListFileNew))); return RTN_ABORT
    ...
    if len(localDeltas) > 0:
        ...
        logging.warning(
            "{:4} file change(s) on LOCAL: {:4} new, {:4} newer, {:4} older, {:4} deleted".format(len(localDeltas),
                                                                                                  news, newers, olders,
                                                                                                  deletes))
    ...
    if len(remoteDeltas) > 0:
        ...
        logging.warning(
            "{:4} file change(s) on REMOTE: {:4} new, {:4} newer, {:4} older, {:4} deleted".format(len(remoteDeltas),
                                                                                                   news, newers, olders,
                                                                                                   deletes))
    ...
    if not force and float(localDeleted) / len(localPrior) > float(MAX_DELETE) / 100:
        logging.error(
            "Excessive number of deletes (>{}%, {} of {}) found on the Local system {} - Aborting. Run with --force if desired."
    ...
    if not force and float(remoteDeleted) / len(remotePrior) > float(MAX_DELETE) / 100:
        logging.error(
            "Excessive number of deletes (>{}%, {} of {}) found on the Remote system {} - Aborting. Run with --force if desired."
    ...
    for key in remoteDeltas:
        ...
        if remoteDeltas[key]['new']:
            ...
            if key not in localNow:
                ...
                if rcloneCmd('moveto', src, dest, switches,
                             linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
        ...
        if remoteDeltas[key]['newer']:
            if key not in localDeltas:
                ...
                dest = localPathBase + key
                logging.info(printMsg("REMOTE", "Copying to local", dest))
                if rcloneCmd('copyto', src, dest, ["--ignore-times"] + switches,
                             linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
            else:
                if key in localNow:
        ...
        if remoteDeltas[key]['deleted']:
            if key not in localDeltas:
                if key in localNow:
                    ...
                    if rcloneCmd('delete', src, switches,
                                 linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
    ...
    if len(remoteDeltas) == 0 and len(localDeltas) == 0 and not first_sync:
        ...
        if rcloneCmd('sync', localPathBase, remotePathBase, excludes + switches,
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL

        logging.info(">>>>> rmdirs Remote")
        if rcloneCmd('rmdirs', remotePathBase, None, switches,
                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL

        if not no_local_delete:
            logging.info(">>>>> rmdirs Local")
            if rcloneCmd('rmdirs', localPathBase, None, switches,
                         linenum=inspect.getframeinfo(inspect.currentframe()).lineno): return RTN_CRITICAL
            ...
            for line in f:
                ...
                if out:
                    ...
                    date_time = time.mktime(
                        datetime.strptime(date + ' ' + _time, '%Y-%m-%d %H:%M:%S').timetuple()) + float('.' + microsec)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--force',
                        help="Bypass MAX_DELETE ({}%%) safety check and run the sync. Also asserts --verbose.".format(
                            MAX_DELETE),
                        action='store_true')
    ...
    remoteFormat = re.compile(
        '([\w-]+):(.*)')  # Handle variations in the Cloud argument -- Remote: or Remote:some/path or Remote:/some/path
    ...
    if out:
        ...
        if remoteName not in clouds:
            logging.error("ERROR Cloud argument <{}> not in list of configured remotes: {}".format(remoteName, clouds));
            exit()
        ...
        logging.error(
            "ERROR Cloud parameter <{}> cannot be parsed. ':' missing? Configured remotes: {}".format(args.Cloud,
                                                                                                      clouds));
        exit()
    ...
    if not os.path.exists(localPathBase):
        logging.error("ERROR LocalPath parameter <{}> cannot be accessed. Path error? Aborting".format(localPathBase));
        exit()
